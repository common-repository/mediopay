...
function mediopayactivate() {
   ...
   $charset_collate = $wpdb->get_charset_collate();
	...
	dbDelta( $sql );
	$table_name = $wpdb->prefix . "mediopay_paidcontent";
   $charset_collate = $wpdb->get_charset_collate();
	$sql = "CREATE TABLE $table_name (
		id mediumint(9) NOT NULL AUTO_INCREMENT,
		postid tinytext NOT NULL,
		paidcontent1 tinytext NOT NULL,
		paidcontent2 tinytext NOT NULL,
		PRIMARY KEY  (id)
	) $charset_collate;";
	require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
	dbDelta( $sql );
...
function mediopay_add_scripts() {
	...
	wp_enqueue_style( 'style', $path . 'mb_style.css?version=1.9' );
	wp_enqueue_script( 'moneybutton', $path . 'moneybutton.js', true);
	wp_enqueue_script( 'bsv', $path . 'bsv.min.js', true);
	wp_enqueue_script( 'scripts_pre_paywall', $path . 'js_pre_paywall.js?version=1.9', true);
	wp_enqueue_script( 'scripts_create_paywall', $path . 'js_create_paywall.js?version=1.9', true);
	wp_enqueue_script( 'scripts_after_paywall', $path . 'js_after_paywall.js?version=1.9',  true);
	//wp_enqueue_script( 'ajax-script', get_template_directory_uri() . '/js/my-ajax-script.js', array('jquery') );
	wp_enqueue_script( 'ajax-script', plugin_dir_url( __FILE__ )  . '/lib/js_pre_paywall.js?version=1.9', array('jquery') );	
