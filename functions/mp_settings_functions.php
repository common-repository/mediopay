...
function mediopay_option_page() {
	...
	if ( isset($myrows[0]->paywhatyouwant)) {
		$mp_paywhatyouwant = $myrows[0]->paywhatyouwant;
	}
	if ( isset($myrows[0]->threshold)) {
		$mp_threshold = $myrows[0]->threshold;
	}
   ...
   ?><form name='setmediopay' method='post' action="<?php esc_url( $_SERVER['REQUEST_URI'] ) ?>">
		...
		<tr>
		<td class="mediopay_column_small">
    	<h2 class="mediopay_options_h2">Style your MedioPay</h2>
   ...
   <input type="text" name="MedioPay_fixed_thank_you" id="MedioPay_fixed_thank_you"  value="<?php if ( isset ( $mp_current_thankyou ) ) echo esc_html($mp_current_thankyou); ?>" /><br />
	...
	<tr>
	<td class="mediopay_column_small"><br />
	<h2 class="mediopay_options_h2">Your Content Economics</h2>
   ...
   <tr>
	<td class="mediopay_column_small">
   <b>Activate pay what you want mode</b>
   <br /></td><td class="mediopay_column_large">
	<label for="MedioPay_paywhatuwant">
	<input type="checkbox" name="MedioPay_paywhatuwant" id="MedioPay_paywhatuwant" value="yes" <?php if ( isset($mp_paywhatyouwant)) { if ($mp_paywhatyouwant == 'yes') { echo "checked";} } ?> "/> 
   </label>
   <br />Let your users decide how much they want to pay to buy your posts.
   </td>
   </tr>
  
	<tr>
	<td class="mediopay_column_small">
	<b>Set a amount to make your posts free</b>
	<br /><br /></td>
	<td class="mediopay_column_large">
	<label for="MedioPay_threshold">
	<input type="number" step="<?php 
		if ($mp_currentcurrency == "BSV") {
		 echo "0.0001";	
	 	}
	...
	?>" name="MedioPay_threshold" id="MedioPay_threshold" value="<?php 
		if (isset($mp_threshold)) { 
			if ($mp_currentcurrency !== "BSV") {
				echo esc_html(round($mp_threshold,2));
			}			
			else {
			}
		}	
   ...
   Set a threshold. If a post reached it with payments for the paywall and for tips, the post will be free for everybody.
   </td>
   </tr>  
   ...
   <h2 class="mediopay_options_h2">Add a second receiver of payments</h2>
	...
	<tr>
    	...
    	/>
    	<br />Enter a second BSV address, paymail address or MoneyButton ID to share the payments. Sharing is deactivated if address is not set. With this option you can co-own the income of this blog.
   ...
   <input type="submit" class="button button-primary" value='save' />
    ...
    <h2 class="mediopay_options_h2">Register for other services</h2>
   <table class="mediopay_table">
   <tr>
		<td class="mediopay_column_small">
			<b>Register for the Faucet on Mediopay.com</b></td>
		<td class="mediopay_column_large">
    	<button onclick="register4faucet()" class="button button-primary">Register for Faucet</button>	
    	<br />Allow your readers to get an initial giveaway of BSV to pay your paywall. The Paywall field will contain a link to the Faucet.<br />
    	To check if you are registered, push the button. It might take a while, don't get nervous.
    	<p id="faucetinfo"></p>
    	</td>
	</tr>
	</table></div>
	<script>
	function register4faucet() {
		console.log("register faucet");
	
	faucet_data = {
	 			'hello': "hello!"
	};
	 console.log(faucet_data);
	 fetch(mp_blogpath + '/wp-json/mediopay/v1/register4faucet/', {
       method: 'POST', // or 'PUT'
       headers: {
           'Content-Type': 'application/json',
       },
       body: JSON.stringify(faucet_data),
     })
    ...
    .then((myJson) => {
       ...
       console.log(responseobj)
		 faucet_data2 = {
			...
			'blogpath': responseobj.blogpath		
		 };
		 console.log ("faucet data2: " + faucet_data2.blogpath + faucet_data2.secret);
		 fetch('https://mediopay.com/wp-json/faucet-api/v1/sayhello/', {
       	method: 'POST', // or 'PUT'
       	headers: {
           'Content-Type': 'application/json',
       	},
       	body: JSON.stringify(faucet_data2),
     	})
    	.then((response) => {
    		console.log(response);
    	...
    	.then((myJson) => {
			...
			console.log(myJson);
  	     	responseobj = JSON.parse(myJson);
  	     	console.log(responseobj);
  	     	document.getElementById("faucetinfo").innerHTML = "<b>" + responseobj + "</b>";		
		})
		.catch((error) => {
      console.error('There has been a problem with your fetch operation:', error);
      document.getElementById("faucetinfo").innerHTML = 'There has been a problem with your fetch operation';
    	})		       
    })
    .catch((error) => {
      console.error('There has been a problem with your fetch operation:', error);
	...
	<!--<br /><br /> <div id="yenpoints"><button onclick="register4YenPoints()">Register for YenPoints</button>-->	
	</div>
	<script>
	/*
	function register4YenPoints() {
		console.log("register yen points");
		faucet_data = {
	 			'hello': "hello yen points!"
	};
	 fetch(mp_blogpath + '/wp-json/mediopay/v1/register4yenpoints/', {
       method: 'POST', // or 'PUT'
       headers: {
           'Content-Type': 'application/json',
       },
    ...
    .then((myJson) => {
       ...
       responseobj = JSON.parse(myJson);
       console.log(responseobj);
       document.getElementById("yenpoints").innerHTML = "<a href='https://www.yenpoint.jp/run-0.4.5-alpha/YenPoint/demo/mediopay.php?url="+responseobj.blogpath + "&secret="+ responseobj.secret +"' target='_blank'>Click to register</a>" ;
	  })		
    .catch((error) => {
      console.error('There has been a problem with your fetch operation:', error);
    })	 
